...
linearcombination :: Z10_2 -> Z10_2 -> Z10_2
linearcombination = zipWith (+)

containsDuplicates :: Eq a => [a] -> Bool
containsDuplicates as = all (<= 1) $ map length $ group as

isVectorSpace :: [Z10_2] -> Bool
isVectorSpace vs = 
  let pairs = map tuplify2 (combinations 2 vs) 
      lcs   = map (uncurry linearcombination) pairs in
  containsDuplicates lcs
...
-- If this crashes with 'Prelude.minimum: empty list', just run it again
main :: IO()
main = do
  rgen <- getStdGen
